...
numOfFoldsPerTrial = 10 # 10 folds or wraps

def evaluatePerformance(numTrials=100): # perform 100 Trials
    '''
    ...
    ** Note that your implementation must follow this API**
    ...
    # Load Data
    filename = '/home/anht/AI_Class/Assignment/CIS419/hw1/SPECTF.dat'
    data = np.loadtxt(filename, delimiter=',')
    y = np.array([data[:, 0]]).T # assign the first column is class label to Y
    X = data[:, 1:] # assign the others to X
    ...
    for x in range(0, numTrials):
        ...
        idx = np.arange(n)
        np.random.seed(13)
        np.random.shuffle(idx) # shuffle array idx [0, ... , 266]
        X = X[idx]
        y = y[idx]

        # split the data randomly into 10 folds
        folds = []    
        intervalDivider = len(X)//numOfFoldsPerTrial # 267 // 10 = 26
        for fold in range(0, numOfFoldsPerTrial):
            # designate a new testing range
            Xtest = X[fold * intervalDivider:(fold + 1) * intervalDivider,:] # Giữ 1 fold để test
            ytest = y[fold * intervalDivider:(fold + 1) * intervalDivider,:]
            Xtrain = X[:(fold * intervalDivider),:] # 9 fold còn lại đưa vào training
    ...
    meanDecisionTreeAccuracy = np.mean(treeAccuracies) # Returns the average of the array elements
    stddevDecisionTreeAccuracy = np.std(treeAccuracies) #Returns the standard deviation, a measure of the spread of a distribution, of the array elements
